...
def edit_metadata(card: dict) -> dict:
    """Prompt for new title / style / category. Empty input keeps current."""
    print()
...
def show_card(card: dict, day_dir: Path) -> None:
    ...
    print(f"  Instrument:  {card['instrumentation']}")
...
def promote_candidate(card: dict, drive: Path, day_dir: Path, catalogue: list[dict]) -> dict:
...
def main() -> None:
    ...
    for card in intaken:
        ...
        while True:
            ...
            if choice == "a":
                row = promote_candidate(card, drive, day_dir, catalogue)
